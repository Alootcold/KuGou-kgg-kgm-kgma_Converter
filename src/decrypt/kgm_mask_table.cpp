#include "kgm_mask_table.h"

QVector<uint8_t> KgmMaskTable::initTable1()
{
    return QVector<uint8_t>({
        
    });
}

QVector<uint8_t> KgmMaskTable::initTable2()
{
    return QVector<uint8_t>({
        
    });
}

QVector<uint8_t> KgmMaskTable::initMaskV2PreDef()
{
    return QVector<uint8_t>({
        
    });
}

const QVector<uint8_t>& KgmMaskTable::getTable1()
{
    static QVector<uint8_t> table = initTable1();
    return table;
}

const QVector<uint8_t>& KgmMaskTable::getTable2()
{
    static QVector<uint8_t> table = initTable2();
    return table;
}

const QVector<uint8_t>& KgmMaskTable::getMaskV2PreDef()
{
    static QVector<uint8_t> table = initMaskV2PreDef();
    return table;
}

uint8_t KgmMaskTable::getMask(size_t pos)
{
    const auto& table1 = getTable1();
    const auto& table2 = getTable2();
    const auto& maskV2PreDef = getMaskV2PreDef();

    size_t offset = pos >> 4;
    uint8_t value = 0;

    while (offset > 0) {
        value ^= table1[offset % 272];
        offset >>= 4;
        if (offset == 0) break;
        value ^= table2[offset % 272];
        offset >>= 4;
    }

    return maskV2PreDef[pos % 272] ^ value;
}
